...
namespace maths {namespace vectors
{
	...
	template <typename T1, typename T2> inline T1 operator*(T1 const& v1, T2 const& v2) {asfd};
	template <typename T1> inline T1 operator*(T1 const& vi, float s);
	template <typename T1> inline T1 operator*(float s, T1 const& vi);
}}

template <typename T1, typename T2> inline float maths::vectors::dot(T1 const& v1, T2 const& v2)
{
	dot(to_archetype(v1), to_archetype(v2));
}

template <typename T1> inline float maths::vectors::magnitude_sqr(T1 const& v)
{
	magnitude_sqr(to_archetype(v));
}

template <typename T1> inline float maths::vectors::magnitude(T1 const& v)
{
	magnitude(to_archetype(v));
}

template <typename T1, typename T2, typename T3> inline void maths::vectors::cross(T1& vo, T2 const& v1, T3 const& v2)
{
	cross(to_archetype(vo), to_archetype(v1), to_archetype(v2));
}

template <typename T1, typename T2> inline void maths::vectors::copy(T1& vo, T2 const& vi)
{
	copy(to_archetype(vo), to_archetype(vi));
}

template <typename T1, typename T2> inline void maths::vectors::add(T1& vo, T2 const& vi)
{
	add(to_archetype(vo), to_archetype(vi));
}

template <typename T1, typename T2, typename T3> inline void maths::vectors::add_copy(T1& vo, T2 const& v1, T3 const& v2)
{
	add_copy(to_archetype(vo), to_archetype(v1), to_archetype(v2));
}

template <typename T1, typename T2> inline void maths::vectors::subtract(T1& vo, T2 const& vi)
{
	subtract(to_archetype(vo), to_archetype(vi));
}

template <typename T1, typename T2, typename T3> inline void maths::vectors::subtract_copy(T1& vo, T2 const& v1, T3 const& v2)
{
	subtract_copy(to_archetype(vo), to_archetype(v1), to_archetype(v2));
}

template <typename T1> inline void maths::vectors::scale(T1& v, float s)
{
	scale(to_archetype(v), s);
}

template <typename T1, typename T2> inline void maths::vectors::scale_copy(T1& vo, T2 const& vi, float s)
{
	scale_copy(to_archetype(vo), to_archetype(vi), to_archetype(s));
}

template <typename T1, typename T2> inline T1 maths::vectors::operator^(T1 const& v1, T2 const& v2)
{
	T1 result;
	cross(result, v1, v2);
	return result;
...
template <typename T1, typename T2> inline T1& maths::vectors::operator+=(T1& vo, T2 const& vi)
{
	add(vo, vi);
	return vo;
}
...
template <typename T1> inline T1& maths::vectors::operator*=(T1& v, float s)
{
	scale(v, s);
	return v;
}

template <typename T1> inline T1 maths::vectors::operator*(T1 const& vi, float s)
{
	T1 result;
...
template <typename T1> inline T1 maths::vectors::operator*(float s, T1 const& vi)
{
	T1 result;
	scale_copy(result, vi, s);
	resturn result;
